...
if [ -f "$PROJECT_ROOT/ariane-xml-c-kernel/build/ariane-xml" ]; then
    ARIANE_XML="$PROJECT_ROOT/ariane-xml-c-kernel/build/ariane-xml"
...
run_test "VERB-001" \
    'SELECT item.name FROM "./ariane-xml-tests/data/truly_ambiguous.xml";' \
    "⚠ Ambiguous attribute\(s\): item\.name"

# Test 2: Non-ambiguous attribute with full path
run_test "VERB-002" \
    'SELECT section.item.name FROM "./ariane-xml-tests/data/truly_ambiguous.xml";' \
    "✓ No ambiguous attributes found"

# Test 3: Ambiguous attribute in company_ambiguous.xml
run_test "VERB-003" \
    'SELECT address.street FROM "./ariane-xml-tests/data/company_ambiguous.xml";' \
    "⚠ Ambiguous attribute\(s\): address\.street"

# Test 4: Non-ambiguous with specific path
run_test "VERB-004" \
    'SELECT store.address.street FROM "./ariane-xml-tests/data/company_ambiguous.xml";' \
    "✓ No ambiguous attributes found"

# Test 5: Top-level attribute (never ambiguous)
run_test "VERB-005" \
    'SELECT name FROM "./ariane-xml-tests/data/company_ambiguous.xml";' \
    "✓ No ambiguous attributes found"

# Test 6: Multiple ambiguous attributes
run_test "VERB-006" \
    'SELECT item.name, item.value FROM "./ariane-xml-tests/data/truly_ambiguous.xml";' \
    "⚠ Ambiguous attribute\(s\): item\.name, item\.value"

# Test 7: Ambiguous in WHERE clause
run_test "VERB-007" \
    'SELECT name FROM "./ariane-xml-tests/data/truly_ambiguous.xml" WHERE item.value > 200;' \
    "⚠ Ambiguous attribute\(s\): item\.value"

# Test 8: Mixed ambiguous and non-ambiguous
run_test "VERB-008" \
    'SELECT section.item.name, item.value FROM "./ariane-xml-tests/data/truly_ambiguous.xml";' \
    "⚠ Ambiguous attribute\(s\): item\.value"
