...
# Test installation
docker compose exec expocli expocli-encrypt --help
```
...
```bash
expocli-encrypt encrypt input.xml output.xml -c config.yaml [-p password]
```

### Decrypt

```bash
expocli-encrypt decrypt encrypted.xml decrypted.xml -c config.yaml [-p password]
```

### View Statistics

```bash
expocli-encrypt stats -c config.yaml [-p password]
```
...
### "expocli-encrypt: command not found"
...
# 2. Test with sample
cd tests/encryption
expocli-encrypt encrypt sample_data.xml test.xml -c ../../my_config.yaml

# 3. Verify
head test.xml

# 4. Decrypt to verify reversibility
expocli-encrypt decrypt test.xml check.xml -c ../../my_config.yaml
diff sample_data.xml check.xml

# 5. Use with real data
expocli-encrypt encrypt ../prod_data.xml ../encrypted_prod.xml -c ../../my_config.yaml
