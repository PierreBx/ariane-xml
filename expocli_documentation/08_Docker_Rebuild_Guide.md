...
### Step 3: Rebuild the ExpoCLI Image
...
```bash
# Check if expocli-encrypt is available
docker compose exec expocli expocli-encrypt --help

# Check if Python packages are installed
docker compose exec expocli python3 -c "import cryptography, faker, lxml; print('All packages installed!')"
```
...
3. ✅ `docker compose exec expocli expocli-encrypt --help` shows the help message
...
3. **Run the tests:**
   ```bash
   docker compose exec expocli python3 -m pytest tests/encryption/test_encryption.py -v
   ```

4. **Try encrypting your own XML files:**
   ```bash
   docker compose exec expocli bash
   expocli-encrypt encrypt input.xml output.xml -c my_config.yaml
   ```
