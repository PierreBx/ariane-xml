# ExpoCLI Quick Start - Command Line Interface

Get started with ExpoCLI in 5 minutes using the command-line interface.
...
```
expocli> SELECT breakfast_menu/food/name FROM ./examples WHERE breakfast_menu/food/calories < 700
Belgian Waffles
French Toast
...
# Find items under $6
expocli "SELECT name,price FROM ./examples WHERE price < 6"

# Query across multiple files
expocli "SELECT FILE_NAME,name FROM ./examples WHERE calories < 700"
```
...
docker compose exec expocli /app/build/expocli
```
