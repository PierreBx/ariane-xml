...
# Now proceed with build/compile
    if [ "$REBUILD_DOCKER" = true ]; then
        ...
        if docker compose exec -T ariane-xml bash -c "rm -rf /app/ariane-xml-c-kernel/build/* /app/ariane-xml-c-kernel/build/.* 2>/dev/null || true"; then
        ...
        if docker compose exec -T ariane-xml bash -c \
            "mkdir -p /app/ariane-xml-c-kernel/build && cd /app/ariane-xml-c-kernel/build && cmake .. >/dev/null 2>&1 && make"; then
            echo ""
...
if [ "$REBUILD_DOCKER" = true ]; then
    ...
    echo "      Persistent container is running and ready!"
    echo "      To rebuild Docker image: ${SCRIPT_DIR}/install.sh --rebuild-docker"
fi
echo ""
echo "Tip:  Check container status: cd ${PROJECT_ROOT} && docker compose ps"
echo "      Stop container: cd ${PROJECT_ROOT} && docker compose down (auto-restarts on next use)"
echo ""
