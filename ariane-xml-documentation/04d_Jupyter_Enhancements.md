...
This document outlines practical enhancements to improve the Ariane-XML Jupyter kernel user experience, prioritized by impact and implementation complexity.

## Current State (POC)

The current kernel provides basic functionality:
- ✅ Execute Ariane-XML queries in cells
- ✅ Display plain text output
- ✅ Basic error handling
- ✅ All Ariane-XML query syntax supported
...
def _create_html_table(self, lines: List[str]) -> str:
    ...
    for i, line in enumerate(lines):
        if 'row(s) returned' in line:
            html.append('</table>')
            html.append(f'<div class="ariane-xml-result-count">{line}</div>')
            break
...
class Ariane-XMLKernel(Kernel):
    def __init__(self, **kwargs):
        ...
        self.ariane-xml_path = self._find_ariane-xml()
...
**Enhancement:** Custom CodeMirror mode for Ariane-XML syntax

**Implementation:** Create `ariane-xml_kernel/codemirror-mode.js`:

```javascript
// Ariane-XML syntax highlighting for CodeMirror
CodeMirror.defineMode("ariane-xml-sql", function() {
...
Update `ariane-xml_kernel/kernelspec/kernel.json`:
...
```python
import unittest
from ariane-xml_kernel import Ariane-XMLKernel

class TestKernelEnhancements(unittest.TestCase):
    def setUp(self):
        self.kernel = Ariane-XMLKernel()
    ...
    def test_html_output_formatting(self):
        ...
        self.assertIn('ariane-xml-table', result['text/html'])
