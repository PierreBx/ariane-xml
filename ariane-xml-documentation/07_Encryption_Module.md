...
The Ariane-XML Encryption Module provides comprehensive encryption and pseudonymization capabilities for XML data, specifically designed for French administrative data. It supports:
...
The encryption module is automatically included in the Ariane-XML Docker image:
...
Create a YAML configuration file to specify which attributes to encrypt. See `ariane-xml-config/encryption_config.example.yaml` for a complete example.
...
The module provides the `ariane-xml-encrypt` command:
...
```python
from ariane-xml_crypto import EncryptionConfig, XMLEncryptor
...
```bash
ariane-xml-encrypt encrypt data.xml encrypted_data.xml -c my_config.yaml
...
```bash
ariane-xml-encrypt decrypt encrypted_data.xml original_data.xml -c my_config.yaml
...
This module is part of the Ariane-XML project.
