...
The Ariane-XML Jupyter kernel allows you to execute Ariane-XML queries in Jupyter notebooks, combining the power of SQL-like XML querying with the interactive notebook environment.
...
✅ **Implemented in POC:**
- Execute Ariane-XML queries in notebook cells
- View query results inline
- Mix queries with Markdown documentation
- Error handling and display
- Support for all Ariane-XML query syntax (SELECT, WHERE, ORDER BY, GROUP BY, etc.)
...
```bash
# Build the Docker image (includes Ariane-XML, Jupyter, and the kernel)
...
Navigate to `examples/Ariane-XML_Demo.ipynb` in the Jupyter Lab interface.
...
1. **Ariane-XML CLI** - Must be installed first
   ```bash
   git clone https://github.com/PierreBx/Ariane-XML
   cd Ariane-XML
   ./install.sh
   ```
...
From the Ariane-XML repository root:
...
You should see `ariane-xml` in the list of available kernels.
...
# 3. Open examples/Ariane-XML_Demo.ipynb and run the cells
```
...
- Click "New" → "Ariane-XML" (in Jupyter Notebook)
- Or select "Ariane-XML" kernel when creating a new notebook (in JupyterLab)
...
Open `examples/Ariane-XML_Demo.ipynb` for a comprehensive tutorial with example queries.
...
2. Executes Ariane-XML as a subprocess
...
```
Ariane-XML/
├── ariane-xml_kernel/              # Kernel package
...
├── examples/
│   └── Ariane-XML_Demo.ipynb      # Demo notebook
...
### Ariane-XML executable not found

The kernel looks for `ariane-xml` in these locations:
- `/usr/local/bin/ariane-xml`
- `/usr/bin/ariane-xml`
- `~/.local/bin/ariane-xml`
- In your PATH

Make sure Ariane-XML CLI is installed and accessible.

### Queries time out

By default, queries have a 30-second timeout. For longer queries, you may need to modify `ariane-xml_kernel/kernel.py` and increase the timeout parameter.
...
# Or manually
python3 -m ariane-xml_kernel.install --uninstall

# Optionally, remove the Python package
pip3 uninstall ariane-xml-kernel
```
...
1. Make changes to `ariane-xml_kernel/kernel.py`
...
- Create custom CodeMirror mode for Ariane-XML syntax
...
- [Ariane-XML Main Documentation](README.md)
...
Same as Ariane-XML main project.

## Support

- GitHub Issues: https://github.com/PierreBx/Ariane-XML/issues
